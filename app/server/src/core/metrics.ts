...
export interface MetricsSummary {
  ...
  io: {
    outputBytes: number;
    inputBytes: number;
    ringbufferLinesDropped: number;
  };
  ws: {
    connections: number; // gauge
    connectTotal: number; // counter
    disconnectTotal: number; // counter
  };
}
...
export const metrics = {
  // Sessions
  incSpawn(pty: boolean) {
    m.spawnsTotal += 1;
    if (pty) m.spawnsPty += 1; else m.spawnsStdio += 1;
    m.sessionsStarting += 1;
  },
  onFirstOutput(ms: number) {
    m.firstOutCount += 1;
    m.firstOutSum += ms;
    if (ms < m.firstOutMin) m.firstOutMin = ms;
    if (ms > m.firstOutMax) m.firstOutMax = ms;
  },
  onRunningTransition() {
    if (m.sessionsStarting > 0) m.sessionsStarting -= 1;
    m.sessionsRunning += 1;
  },
  onExit(code: number | null | undefined) {
    if (m.sessionsRunning > 0) m.sessionsRunning -= 1;
    m.sessionsExited += 1;
    m.exitsTotal += 1;
    if (code && code !== 0) m.exitsWithError += 1;
  },

  // IO
  addOutputBytes(n: number) { m.outBytes += n; },
  addInputBytes(n: number) { m.inBytes += n; },
  addDroppedLines(n: number) { if (n > 0) m.linesDropped += n; },

  // WS
  wsConnect() { m.wsConnections += 1; m.wsConnectTotal += 1; },
  wsDisconnect() { if (m.wsConnections > 0) m.wsConnections -= 1; m.wsDisconnectTotal += 1; },
